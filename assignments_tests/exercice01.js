  ...
  describe("page3.html", () => {
    ...
    it("Sous-sections de la page", async () => {
      ...
      it("Liste alphabétique", async () => {
        const nb = await page.evaluate(() => [...document.querySelectorAll('ol[type="a"]')].map(ol => ol.children.length));
        expect(nb).toEqual([4]);
      });

      it("Liste à puces", async () => {
        const nb = await page.evaluate(() => [...document.querySelectorAll('ul')].map(ol => ol.children.length));
        expect(nb).toEqual([7]);
      });
